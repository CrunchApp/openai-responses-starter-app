-- Migration: 20250523200000_fix_decimal_duration.sql
-- Fix issue with decimal durations by changing duration to NUMERIC type and updating store_recommendation

-- First, alter the duration column in programs table
ALTER TABLE programs ALTER COLUMN duration TYPE NUMERIC USING duration::NUMERIC;

-- Update store_recommendation function to handle decimal durations
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Default to 70 if can't parse as integer
    v_match_score := 70;
  ...
  -- Handle cost_per_year safely
  BEGIN
    v_cost_per_year := (p_recommendation->>'costPerYear')::NUMERIC;
  EXCEPTION WHEN OTHERS THEN
    -- Try to extract numbers only and convert
    BEGIN
