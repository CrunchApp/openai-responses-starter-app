-- Fix ambiguous column reference in store_recommendation function
...
BEGIN
  ...
  IF v_program_name IS NULL OR v_program_name = '' THEN
    RETURN QUERY SELECT FALSE, NULL::UUID, NULL::UUID, 'Program name is required';
    RETURN;
  END IF;
  
  IF v_program_institution IS NULL OR v_program_institution = '' THEN
    RETURN QUERY SELECT FALSE, NULL::UUID, NULL::UUID, 'Institution name is required';
    RETURN;
  END IF;
  ...
  BEGIN
    ...
    -- FIX: Add table alias 'r' to avoid ambiguous column reference
    DELETE FROM recommendations r
    WHERE r.user_id = p_user_id AND r.program_id = v_program_id AND r.pathway_id IS NOT DISTINCT FROM p_pathway_id;
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT FALSE, NULL::UUID, NULL::UUID, 'Error cleaning up existing recommendation: ' || SQLERRM;
    RETURN;
  END;
  ...
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT FALSE, NULL::UUID, v_program_id, 'Error creating recommendation: ' || SQLERRM;
    RETURN;
  END;

  -- Return success with both IDs and an indication if this was a duplicate program
  IF v_duplicate_program THEN
    RETURN QUERY SELECT TRUE, v_recommendation_id, v_program_id, 'Used existing program with same name and institution';
  ELSE
    RETURN QUERY SELECT TRUE, v_recommendation_id, v_program_id, NULL::TEXT;
  END IF;
END;
$$; 