...
export default function RecommendationsPage() {
  ...
  // Local state to track favorited recommendations for display
  const [favoritedRecommendations, setFavoritedRecommendations] = useState<RecommendationProgram[]>([]);
    ...
    async function fetchRecommendations() {
      if (vectorStore && vectorStore.id) {
        try {
          ...
          // Check if we already have recent recommendations in the store
          const CACHE_DURATION = 3600000; // 1 hour in milliseconds
          const isCacheValid = 
            recommendations.length > 0 && 
            lastRecommendationsUpdate > 0 &&
            Date.now() - lastRecommendationsUpdate < CACHE_DURATION;
          ...
          if (isCacheValid) {
            ...
            setFavoritedRecommendations(
              recommendations.filter(rec => rec.isFavorite)
            );
            return;
          }
          ...
          const result = await generateRecommendations(
            vectorStore.id, 
            profileData // Use profile data from the store
          );
          ...
          if (result.error) {
            ...
            // Update recommendations in the store
            setRecommendations(result.recommendations);
            
            // Update local state for favorites view
            setFavoritedRecommendations(
              result.recommendations.filter(rec => rec.isFavorite)
            );
          } else {
  ...
  const handleToggleFavorite = (recommendationId: string) => {
    ...
    toggleFavoriteRecommendation(recommendationId);
    ...
    setFavoritedRecommendations(updatedRecs.filter(rec => rec.isFavorite));
  };
          ...
          <div className="mb-6">
            ...
            >
              <h1 className="text-2xl md:text-3xl font-bold mb-2">
                Hi {profileData.firstName}, welcome to your education dashboard
              </h1>
              <p className="text-zinc-600">
                Exploring {profileData.goal} programs in {profileData.desiredField} based on your profile
              </p>
            </motion.div>
          </div>
              ...
              <div className="bg-white p-6 rounded-lg shadow-md mb-6">
                ...
                {favoritedRecommendations.length > 0 ? (
                  <div className="space-y-4">
                    {favoritedRecommendations.map(program => (
